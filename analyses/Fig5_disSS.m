...
%% Get data
data = readtable(sprintf('%s/stats/data/tazoot_sdt_kt.csv',zootDir));
...
for iPanel = 1 % :2
    ...
    for iT=1:2 % Targets
        ...
        iNT=1;
        for iV=1:3
            % Create logical index for rows
            idx = data.validity == iV & data.target == iT & data.nonTargetContrast==iNT;
            % Select those rows, all columns
            subset = data(idx, :);
            % Get just the column you want
            values = data{idx, 'dis_dp_SS'};
            
            yVals_all(:,iV) = values;

            n = size(values,1);

            ste(:,iV) = std(values,[],1) / sqrt(n);
        end
        xValsC = xVals+0.06;
        meanValues = mean(yVals_all,1); 
        pl(count) = plot(xValsC(iT,:),meanValues,'LineWidth',0.5,'Color','k');
        count = count+1; 
        seb = zoot_shadedErrorBar(xValsC(iT,:),meanValues,mean(ste,1));
        seb.patch.FaceColor = fp.grey;
        for iV=1:3 % attention
            sc = scatter(xValsC(iT,iV),meanValues(iV),fp.markerSize+3,'filled');
        ...
        seb.patch.FaceColor = fp.grey;
        for iV=1:3 % attention
            sc = scatter(xValsC(iT,iV),meanValues(iV),fp.markerSize);
