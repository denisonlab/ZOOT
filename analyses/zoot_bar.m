...
if figNum == 2 || figNum == 3 % for the accuracy plots, TP=filled, TA=hollow
    b = bar(mean(x), mean(y,'omitnan'),...
        ...
        'EdgeColor',fp.colorsTargets(iT,iV,:),'FaceColor',squeeze(fp.bar.FaceColor(iC,iT,iV,:)));
...
elseif figNum==6 || figNum==7 % for swap, all grey
    b = bar(mean(x), mean(y,'omitnan'),...
        'LineWidth',fp.bar.lineWidth(iC),'BarWidth',fp.bar.barWidth(iC),... 
...
if plotError
    eb = errorbar(x(1),mean(y,'omitnan'),err,'k','CapSize',fp.CapSize,'LineWidth',fp.ErrorBarLineWidth); 
end
