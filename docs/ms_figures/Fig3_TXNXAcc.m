...
for iPanel = 1:2
    ...
    for iT=1:2
        ...
        xValsC = xVals+0.06; 
        pl = plot(xValsC(iT,:),yValsMean1,'LineWidth',1.5,'Color','k');
        for iV=1:3 % attention
            sc = scatter(xValsC(iT,iV),yValsMean1(iV),'filled');
            sc.MarkerFaceColor = fp.colorsTargets(iT,iV,:);
        end
        seb = shadedErrorBar(xValsC(iT,:),yValsMean1,ste);
        seb.patch.FaceColor = 'k'; 
        
        % Without competition
        yValsMean2 = mean(yVals2,2);
        ste = std(yVals2,[],2) / sqrt(n);
        pl = plot(xVals(iT,:),yValsMean2,'LineWidth',1.5,'Color',[0.7 0.7 0.7]);
        for iV=1:3 % attention
            sc = scatter(xVals(iT,iV),yValsMean2(iV));
            sc.MarkerFaceColor = 'w'; 
            sc.MarkerEdgeColor = fp.colorsTargets(iT,iV,:);
            sc.LineWidth = 1.5; 
        end
        seb = shadedErrorBar(xVals(iT,:),yValsMean2,ste);
        seb.patch.FaceColor = [0.7 0.7 0.7];
        % for iE=1:2
        %     seb.edge(iE).LineWidth = 0; 
        % end
    end
end

% for iT = 1:2
%     yVals = squeeze(Acc_scatterplot.(contrasts{iC})(:,:,iT));
%     yValsMean = mean(yVals,2);
%     pl = plot(xVals(iT,:),yValsMean,'LineWidth',1.5);
% end
