...
%% Orientation pseudorandomization
    % T1 orientation (rotation angle)
    T1Oris = [];
    T2Oris = [];
    for iT1Axis=1:numel(p.axes)
        for iT1Tilt=1:numel(p.tilts)
            T1Ori = p.axes(iT1Axis) + threshold*p.tilts(iT1Tilt); % creates the four different orientations
            T1Oris = [T1Oris T1Ori]; % accumulates orientations in a list
        end 
    end 

    % T2 orientation (rotation angle)
    for iT2Axis = 1:numel(p.axes)
        for iT2Tilt = 1:numel(p.tilts)
            T2Ori = p.axes(iT2Axis) + threshold*p.tilts(iT2Tilt);
            T2Oris = [T2Oris T2Ori];
        end
    end
...
for iTrial = trialCounter:p.nTotalTrials % 1280 p.nTrialsPerBlock % the iteration in the trial loop
    ...
    %Tilt: randomly selected per trial
    ...
    if T2Orientation == 1 || T2Orientation == 3
        T2Tilt = p.tilts(1);
    else
        T2Tilt = p.tilts(2);
    end
