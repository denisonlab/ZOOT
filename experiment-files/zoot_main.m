...
if s.exptStage == 4 || s.exptStage == 5
    if s.session == 1
        if isempty(dataFileNames)
            ...
            iTrialskipped = dataPrevious.data.iTrialskipped_ses1;
            skippedTrials = dataPrevious.data.skippedTrials_ses1;
    ...
    elseif s.session == 2
        ...
        if isempty(findSes2File)
         ...
         data = dataPrevious.data;
            ...
            iTrialskipped = dataPrevious.data.iTrialskipped_ses2;
            skippedTrials = dataPrevious.data.skippedTrials_ses2;
            eyeSkip = dataPrevious.data.eyeSkip; % trials skipped due to an eye movement, same size as trials matrix
        end
    else 
...
while iTrial <= size(sesTrialOrder, 2)
   ...
   if p.eyeTracking
     ...
     if p.eyeTracking
        while GetSecs < timePrecue + p.precueSOA - p.eyeSlack && ~stopThisTrial
            ...
            if fixation==0
                ...
                % trialOrder
                sesTrialOrder(end+1) = sesTrialOrder(iTrial);
                skippedTrials(end+1) = sesTrialOrder(iTrial);
    ...
    if mod(iTrial, p.nTrialsPerBlock)==0 || iTrial == size(sesTrialOrder,2)
        ...
        if s.session == 1
            data.skippedTrials_ses1 = skippedTrials;
            data.iTrialskipped_ses1 = iTrialskipped;
        elseif s.session == 2
            data.skippedTrials_ses2 = skippedTrials;
            data.iTrialskipped_ses2 = iTrialskipped;
        end 
