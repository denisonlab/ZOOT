...
for iSub=1:length(subs) % for participant
   ...
   %% variables
        ...
        contrastConds = {PresentPresent PresentAbsent AbsentPresent AbsentAbsent};
            ...
            %% accuracy - including idx by nontarget
        ...
        for iTarget = 1:2 % for each target (1 or 2)
            for iContrastCond = 1:4 % for each contrast condition (PP, PA, AP, AA)
                for iValidity = 1:3 % for each precue validity (Valid, Neutral, Invalid)
                    ...
                    % idxNTP = dataAll(iSub).target == iTarget & Validities{iValidity} & dataAll(iSub).nonTargetContrast == 1 & ~dataAll(iSub).eyeSkip; % idx for nontarget present trials
                    % idxNTA = dataAll(iSub).target == iTarget & Validities{iValidity} & dataAll(iSub).nonTargetContrast == 0 & ~dataAll(iSub).eyeSkip; % idx for nontarget absent trials
                    % 
                    ...
                    % 
                    % Acc.nNTP(iValidity, iTarget) = size(dataAll(iSub).correct(idxNTP),2); % denominator, number of trials per condition
                    % Acc.correctNTP(iValidity, iTarget) = size(dataAll(iSub).correct(idxNTP & dataAll(iSub).correct==1),2); % numerator, number of trials that meet a certain rule (correct, seen, correctDis, RT)
                    % Acc.propNTP(iValidity, iTarget) = Acc.correctNTP(iValidity, iTarget)/Acc.nNTP(iContrastCond, iValidity, iTarget);
                    % 
                    % Acc.nNTA(iValidity, iTarget) = size(dataAll(iSub).correct(idxNTA),2); % denominator, number of trials per condition
                    % Acc.correctNTA(iValidity, iTarget) = size(dataAll(iSub).correct(idxNTA & dataAll(iSub).correct==1),2); % numerator, number of trials that meet a certain rule (correct, seen, correctDis, RT)
                    % Acc.propNTA(iValidity, iTarget) = Acc.correctNTA(iValidity, iTarget)/Acc.nNTA(iContrastCond, iValidity, iTarget);
                end
            end
        end
        ...
        for iTarget = 1:2
            for iValidity = 1:3 % for each precue validity (Valid, Neutral, Invalid)
                ...
                Acc.nNTP(iTarget, iValidity) = size(dataAll(iSub).correct(idxNTP),2); % denominator, number of trials per condition
                Acc.correctNTP(iTarget, iValidity) = size(dataAll(iSub).correct(idxNTP & dataAll(iSub).correct==1),2); % numerator, number of trials that meet a certain rule (correct, seen, correctDis, RT)
                Acc.propNTP(iTarget, iValidity) = Acc.correctNTP(iTarget, iValidity)/Acc.nNTP(iTarget, iValidity);

                Acc.nNTA(iTarget, iValidity) = size(dataAll(iSub).correct(idxNTA),2); % denominator, number of trials per condition
                Acc.correctNTA(iTarget, iValidity) = size(dataAll(iSub).correct(idxNTA & dataAll(iSub).correct==1),2); % numerator, number of trials that meet a certain rule (correct, seen, correctDis, RT)
                Acc.propNTA(iTarget, iValidity)= Acc.correctNTA(iTarget, iValidity)/Acc.nNTA(iTarget, iValidity);
            end
        end


        dataAll(iSub).means = Acc.prop*100; % save each participant's mean data in dataAll
        dataAll(iSub).meansNTP = Acc.propNTP*100;
        dataAll(iSub).meansNTA = Acc.propNTA*100;
...
accNTPIdx = [];
accNTAIdx = [];
% accs = {'acc' 'accNTA' 'accNTA'};
% % means = {'means', 'meansNTP', 'meansNTA'};
% % for iAccs = 1:numel(accs)
    ...
    for iValidity = 1:numel(Validities)
        for iTarget = 1:2
            for iSub = 1:length(subs)
                ...
                accNTPIdx = [accNTPIdx dataAll(iSub).meansNTP(iTarget, iValidity)];
                accNTAIdx = [accNTAIdx dataAll(iSub).meansNTA(iTarget, iValidity)];
            end
            % acc.std(iValid,iTarget) = std(accIdx); % finds std of the accuracy of each condition for each participant
            % acc.mean(iValid, iTarget) = mean(accIdx); % finds means of accuracy for each condition for each participant
            % acc.err(iValid,iTarget) = acc.std(iContrast,iValid,iTarget)/sqrt(size(dataAll,2)); % calculate error for each condition

            accNT.pStd(iTarget, iValidity)= std(accNTPIdx); % finds std of the accuracy of each condition for each participant
            accNT.pMean(iTarget, iValidity) = mean(accNTPIdx); % finds means of accuracy for each condition for each participant
            accNTErr.nontargetPresent(iTarget, iValidity) = accNT.pStd(iTarget, iValidity)/sqrt(size(dataAll,2)); % calculate error for each condition

            accNT.aStd(iTarget, iValidity) = std(accNTAIdx); % finds std of the accuracy of each condition for each participant
            accNT.aMean(iTarget, iValidity) = mean(accNTAIdx); % finds means of accuracy for each condition for each participant
            accNTErr.nontargetAbsent(iTarget, iValidity) = accNT.aStd(iTarget, iValidity)/sqrt(size(dataAll,2)); % calculate error for each condition
        end
    end
% end
% end

%% plot acc by nontarget
figure();
sgtitle('accuracy by nontarget')
ntFieldnames = fieldnames(accNTErr);
ntAccs = cat(3, accNT.pMean, accNT.aMean);
for iF = 1:2 % for two subplots
    subplot(1,2,iF)
    b = bar(ntAccs(:,:,iF));
    hold on
    for k = 1:numel(b)      % code to align error bars to grouped subplot bar coordinate, revised from stack exchange   % Recent MATLAB Versions
        ...
        errorbar(xtips,ytips,accNTErr.(ntFieldnames{iF})(:,k), '.k', 'MarkerSize',0.1)
    end
    hold off
    title([ntFieldnames{iF}])
    ylabel('accuracy %')
    ylim([30 100])
