...
%% compile
subs = {'S0001','S0004', 'S0005', 'S0007', 'S0013', 'S0015', 'S0018', 'S0019', 'S0070', 'S0071', 'S0085', 'S0105', 'S0108', 'S0111', 'S0122', 'S0133', 'S0135', 'S0149'};
dataAll = [];
        ...
        for inonTarget = 1:2 % for each target (1 or 2)
            for iContrastCond = 1:4 % for each contrast condition (PP, PA, AP, AA)
                for iValidity = 1:3 % for each precue validity (Valid, Neutral, Invalid)
                    ...
                    ntAcc.correct(iContrastCond, iValidity, inonTarget) = size(dataAll(iSub).NTcorrect(idx & dataAll(iSub).NTcorrect==1),2); % numerator, number of trials that a swap occurred)
...
%% swap rate over all trials chance 
% %gives percent accuracy for when the target and non-target are different
% %only 
% % for iTarget = 1:2 % for each target (1 or 2)
% %     for iContrastCond = 1:4 % for each contrast condition (PP, PA, AP, AA)
% %         for iValidity = 1:3 % for each precue validity (Valid, Neutral, Invalid)
% %             idx = dataAll(iSub).target == iTarget & Validities{iValidity} & contrastConds{iContrastCond} & dataAll(iSub).targetTilt ~= dataAll(iSub).nonTargetTilt & ~dataAll(iSub).eyeSkip;
% %             chanceAcc.n(iContrastCond, iValidity, iTarget) = size(dataAll(iSub).correct(idx),2); % denominator, number of trials per condition
% %             chanceAcc.correct(iContrastCond, iValidity, iTarget) = size(dataAll(iSub).correct(idx & dataAll(iSub).correct==1),2); % numerator, number of trials that meet a certain rule (correct, seen, correctDis, RT)
% %             chanceAcc.prop(iContrastCond, iValidity, iTarget) = chanceAcc.correct(iContrastCond, iValidity, iTarget)/chanceAcc.n(iContrastCond, iValidity, iTarget);
% %         end
% %     end
% % % end
% % 
...
%             % end 
%             chanceSwap_rev(iContrastCond, iValidity, inonTarget) = (100-dataAll(iSub).means(iContrastCond, iValidity, 3-inonTarget))/2;
%         end 
%     end
% end
% dataAll(iSub).chanceSwap(:,:,1) = chanceSwap_rev(:,:,2);
% dataAll(iSub).chanceSwap(:,:,2) = chanceSwap_rev(:,:,1);
...
%% swapping rate over incorrect trials (incorrect trials where non-target reported / incorrect trials)
    ...
    %% RT TP/TA
        ...
        %% num error per cond per participant
        for iContrast = 1:numel(contrastConds)
            for iValid = 1:numel(Validities)
                for iTarget = 1:2
                        idx = dataAll(iSub).target==iTarget & dataAll(iSub).correct==0 & Validities{iValid} & contrastConds{iContrast} & ~dataAll(iSub).eyeSkip;
                        dataAll(iSub).numIncorrectTrials(iContrast, iValid,iTarget)= size(find(idx==1),2);
                end
            end
        end
    ...
    for iTarget = 1:2
        for iValid = 1:numel(Validities)
            for iF = 1:numel(Disfieldnames)
                ...
                % discrimination d' for loglinear corrections for all trials
    ...
    for iTarget = 1:2
        for iF = 1:numel(Detfieldnames)
            ...
            [dprime, criterion] = kt_dprime2(dataAll(iSub).detNHColl.(Detfieldnames{iF})(1, iTarget), dataAll(iSub).detNFAColl.(Detfieldnames{iF})(1, iTarget), dataAll(iSub).detNSignalColl.(Detfieldnames{iF})(1, iTarget), dataAll(iSub).detNNoiseColl.(Detfieldnames{iF})(1, iTarget),'over2N');
    ...
    for iTarget = 1:2
        for iF = 1:numel(Disfieldnames)
            ...
            [dprime, criterion] = kt_dprime2(dataAll(iSub).nhDisColl.(Disfieldnames{iF})(1,iTarget), dataAll(iSub).nfaDisColl.(Disfieldnames{iF})(1,iTarget), dataAll(iSub).nsignalDisColl.(Disfieldnames{iF})(1,iTarget), dataAll(iSub).nnoiseDisColl.(Disfieldnames{iF})(1,iTarget),'over2N');
...
%or invalid which could be inflating valid det d' values
        ...
        for iTarget = 1:2
            for iValid = 1:numel(Validities)
                for iF = 2:numel(Detfieldnames)
                    ...
                    % det subsampled all loglinear
                    [dprime, criterion] = kt_dprime2(dataAll(iSub).detNH_sub.(Detfieldnames{iF})(iTarget,iValid), dataAll(iSub).detNFA_sub.(Detfieldnames{iF})(iTarget,iValid), dataAll(iSub).detNSignal_sub.(Detfieldnames{iF})(iTarget,iValid), dataAll(iSub).detNNoise_sub.(Detfieldnames{iF})(iTarget,iValid),'loglinear');
                    dataAll(iSub).detdprime_sub.(Detfieldnames{iF})(iTarget,iValid,iRep) = dprime; % store d prime
                    dataAll(iSub).detcrit_sub.(Detfieldnames{iF})(iTarget,iValid, iRep) = criterion; % store c
                    % det subsampled over2N
                    ...
                    dataAll(iSub).detdprime_sub_over2N.(Detfieldnames{iF})(iTarget,iValid,iRep) = dprime; % store d prime
                    dataAll(iSub).detcrit_sub_over2N.(Detfieldnames{iF})(iTarget,iValid, iRep) = criterion; % store c
                    %dis
                    dataAll(iSub).disNH_sub.(Detfieldnames{iF})(iTarget,iValid) = dis_subsample.(Detfieldnames{iF})(iTarget,iValid,1);
                    ...
                    dataAll(iSub).disdprime_sub_over2N.(Detfieldnames{iF})(iTarget,iValid,iRep) = dprime; % store d prime
                    dataAll(iSub).discrit_sub_over2N.(Detfieldnames{iF})(iTarget,iValid, iRep) = criterion; % store c
                end
            end
        end
    end
    for iF = 2:numel(Detfieldnames)
        %det all loglinear
        dataAll(iSub).avg_detd_sub.(Detfieldnames{iF}) = mean(dataAll(iSub).detdprime_sub.(Detfieldnames{iF})(:,1,:),3);
        ...
        dataAll(iSub).avg_detc_sub.(Detfieldnames{iF}) = mean(dataAll(iSub).detcrit_sub.(Detfieldnames{iF})(:,1,:),3);
        ...
        dataAll(iSub).disd_sub.(Detfieldnames{iF})(:,2:3) = dataAll(iSub).disd.(Detfieldnames{iF})(:,2:3);

        dataAll(iSub).avg_disc_sub.(Detfieldnames{iF}) = mean(dataAll(iSub).discrit_sub.(Detfieldnames{iF})(:,1,:),3);
        dataAll(iSub).disc_sub.(Detfieldnames{iF})(:,1) = dataAll(iSub).avg_disc_sub.(Detfieldnames{iF});
        dataAll(iSub).disc_sub.(Detfieldnames{iF})(:,2:3) = dataAll(iSub).disc.(Detfieldnames{iF})(:,2:3);
        % dis over2N
...
%% accuracy of nontarget (swapping) over all trials and incorrect trials (with no exlcusions based on target accuracy)
...
for iContrast = 1:numel(contrastConds)
    for iValid = 1:numel(Validities)
        for inonTarget = 1:2
            for iSub = 1:length(subs)
                accIdx = [accIdx dataAll(iSub).NTAccmeans(iContrast,iValid,inonTarget)]; % swapping over all trials, collects the accuracy of each condition by each participant into a list so can do group analysis 
                accErrIdx = [accErrIdx dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget)]; % swapping over incorrect trials
                numIncorrectIdx = [numIncorrectIdx dataAll(iSub).numIncorrectTrials(iContrast,iValid,inonTarget)];
                ...
                numIncorrect_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = dataAll(iSub).numIncorrectTrials(iContrast,iValid,inonTarget);
            ...
            Acc.numIncorrectStd(iContrast,iValid,inonTarget) = std(numIncorrectIdx); % finds std of the accuracy of each condition for each participant
            Acc.numIncorrectMean(iContrast, iValid, inonTarget) = mean(numIncorrectIdx); % finds means of accuracy for each condition for each participant
             Acc.numIncorrectMedian(iContrast, iValid, inonTarget) = median(numIncorrectIdx); % finds means of accuracy for each condition for each participant
            Acc.NTErrerr(iContrast,iValid,inonTarget) = Acc.numIncorrectStd(iContrast,iValid,inonTarget)/sqrt(size(dataAll,2)); % calculate error for each condition 
            numIncorrectIdx = [];
...
%% swap/incorrect exclude participants with over 93.75% accuracy
excludeSub = []; % participants that have over 95% target accuracy in any condition except TANA
includeSub = []; % participants that have under 95% target accuracy in all conditions except TANA
excludeSID = []; % participants that have over 95% target accuracy in any condition except TANA
includeSID= []; % participants that have under 95% target accuracy in all conditions except TANA
for iSub=1:length(subs)
    if size(find(dataAll(iSub).means(1:3,:,:) > 93.75),1) > 0
        excludeSID = [excludeSID subs(iSub)];
        excludeSub = [excludeSub iSub];
    else 
        includeSID = [includeSID subs(iSub)];
        includeSub = [includeSub iSub];
    end 
end 
...
for iContrast = 1:numel(contrastConds)
    for iValid = 1:numel(Validities)
        for inonTarget = 1:2
            for iSub = 1:length(includeSub)
               partsUnder95_Idx = [partsUnder95_Idx dataAll(includeSub(iSub)).NTErrAccmeans(iContrast,iValid,inonTarget)];
               partsUnder95_swap_scatterplot.(contrasts{iContrast})(iValid,iSub,inonTarget)=dataAll(includeSub(iSub)).NTErrAccmeans(iContrast,iValid,inonTarget);
            end
           partsUnder95_swapStd(iContrast, iValid, inonTarget) = std(partsUnder95_Idx);
           partsUnder95_swapMeans(iContrast, iValid, inonTarget) = mean(partsUnder95_Idx);
           partsUnder95_swapErr(iContrast, iValid, inonTarget) = partsUnder95_swapStd(iContrast, iValid, inonTarget) / sqrt(size(includeSub,2));
           partsUnder95_Idx=[];
        end
    end
end
        ...
        for inonTarget = 1:2
            for iSub = 1:length(subs)
                if dataAll(iSub).means(iContrast,iValid,inonTarget)> 93.75 %if target accuracy is greater than 93.75 (2 incorrect trials for neutral/invalid, 6 for valid), omit from swap analysis
                    ...
                    pointsUnder95_swap_scatterplot.(contrasts{iContrast})(iValid,iSub,inonTarget)=dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget);
                    numIncorrect_pointsUnder95_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = dataAll(iSub).numIncorrectTrials(iContrast,iValid,inonTarget);
                ...
                if dataAll(iSub).means(iContrast,iValid,inonTarget)< 93.76 && dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget) == 100 || dataAll(iSub).means(iContrast,iValid,inonTarget)< 93.76 && dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget) == 0%|| dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget) == 0 %if target accuracy is less than 93.75 and swap rate is 100%, state accuracy (this is to make sure that 100% swap rate isn't due to low incorrect trial count and chance)
                    allswaps_Under95_swap_scatterplot.(contrasts{iContrast})(iValid,iSub,inonTarget)=dataAll(iSub).NTErrAccmeans(iContrast,iValid,inonTarget); 
                    acc_pointsUnder95_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = dataAll(iSub).means(iContrast,iValid,inonTarget);
                    numIncorrect_pointsUnder95_100or0_swap.(contrasts{iContrast})(iValid,iSub, inonTarget) = dataAll(iSub).numIncorrectTrials(iContrast,iValid,inonTarget); %need num incorrect here if want to plot incorrect trials for all data points X% target accuracy and less with swap rates of 100% and 0%
                else
                    acc_pointsUnder95_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = NaN;
                    numIncorrect_pointsUnder95_100or0_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = NaN;
                    allswaps_Under95_swap_scatterplot.(contrasts{iContrast})(iValid,iSub,inonTarget)=NaN;
                end
            end
            pointsUnder95_swapStd(iContrast, iValid, inonTarget) = std(pointsUnder95_Idx, "omitmissing");
            pointsUnder95_swapMeans(iContrast, iValid, inonTarget) = nanmean(pointsUnder95_Idx);
           pointsUnder95_swapErr(iContrast, iValid, inonTarget) = pointsUnder95_swapStd(iContrast, iValid, inonTarget) / sqrt(size(pointsUnder95_Idx,2));
           pointsUnder95_Idx=[];
        end
    end
end
...
for iContrast = 1:numel(contrastConds)
    for iValid = 1:numel(Validities)
        for inonTarget = 1:2
            for iSub = 1:length(subs)
                if iValid == 1
                    if dataAll(iSub).means(iContrast,iValid,inonTarget)> 97.92 %if target accuracy is greater than 97.92 (2 incorrect trials for valid), omit from swap analysis
                    ...
                    if dataAll(iSub).means(iContrast,iValid,inonTarget)> 93.75 %if target accuracy is greater than 93.75 (2 incorrect trials for neutral/invalid), omit from swap analysis
                        atLeastTwoIncorrect = [atLeastTwoIncorrect NaN];
                        atLeastTwoIncorrect_swap_scatterplot.(contrasts{iContrast})(iValid,iSub,inonTarget) = NaN;
                        numIncorrect_atLeastTwoIncorrect_swap.(contrasts{iContrast})(iValid, iSub, inonTarget) = NaN; % need numIncorrect here if want to plot num incorrect trials to all data points X% target accuracy and less
                ...
                nan = isnan(atLeastTwoIncorrect);
                nonNaN = atLeastTwoIncorrect(~nan);
                atLeastTwoIncorrect_swapErr(iContrast, iValid, inonTarget) = atLeastTwoIncorrect_swapStd(iContrast, iValid, inonTarget) / sqrt(size(nonNaN,2));
                atLeastTwoIncorrect=[];
        end
    end
end
...
%         for inonTarget = 1:2
%             for iSub = 1:length(subs)
%                 accChanceIdx = [accChanceIdx dataAll(iSub).chanceSwap(iContrast,iValid,inonTarget)]; % chance of swap calc
% 
%                 swap_Chance_scatterplot.(contrastsChance{iContrast})(iValid,iSub,inonTarget)=dataAll(iSub).chanceSwap(iContrast,iValid, inonTarget);
%             end
%             Acc.chanceNTStd(iContrast,iValid,inonTarget) = std(accChanceIdx); % finds std of the accuracy of each condition for each participant
%             Acc.chanceNTMean(iContrast, iValid, inonTarget) = mean(accChanceIdx); % finds means of accuracy for each condition for each participant
%             Acc.chanceNTErr(iContrast,iValid,inonTarget) = Acc.chanceNTStd(iContrast,iValid,inonTarget)/sqrt(size(dataAll,2)); % calculate error for each condition 
%             accChanceIdx = [];
%         end
%     end
% end
     ...
     for iF = 1:numel(Detfieldnames)
         for iValid = 1:numel(Validities)
             for iSub = 1:length(subs)
                 ...
                 %dis extreme loglinear correction
                 extreme_dprimeDisIdx = [dprimeDisIdx dataAll(iSub).disd_extremeLogLin.(Detfieldnames{iF})(iTarget,iValid)];
                 ...
                 %det over2N correction
                 over2N_dprimeDetIdx = [over2N_dprimeDetIdx dataAll(iSub).detd_over2N.(Detfieldnames{iF})(iTarget,iValid)];
                 disd_over2N_scatterplot.(Detfieldnames{iF})(iValid, iSub, iTarget) = dataAll(iSub).detd_over2N.(Detfieldnames{iF})(iTarget,iValid);
                 over2N_cDetIdx = [over2N_cDetIdx dataAll(iSub).detc_over2N.(Detfieldnames{iF})(iTarget,iValid)];
                 disc_over2N_scatterplot.(Detfieldnames{iF})(iValid, iSub, iTarget) = dataAll(iSub).detc_over2N.(Detfieldnames{iF})(iTarget,iValid);
             ...
             detdStd_over2N.(Disfieldnames{iF})(iTarget, iValid) = std(over2N_dprimeDetIdx);
             detd_over2N.(Disfieldnames{iF})(iTarget, iValid) = mean(over2N_dprimeDetIdx);
             detdErr_over2N.(Disfieldnames{iF})(iTarget, iValid) = detdStd_over2N.(Disfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
             detcStd_over2N.(Disfieldnames{iF})(iTarget, iValid) = std(over2N_cDetIdx);
             detc_over2N.(Disfieldnames{iF})(iTarget, iValid) = mean(over2N_cDetIdx);
             detcErr_over2N.(Disfieldnames{iF})(iTarget, iValid) = detcStd_over2N.(Disfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
             ...
             extreme_disdStd.(Disfieldnames{iF})(iTarget, iValid) = std(extreme_dprimeDisIdx);
             extreme_disd.(Disfieldnames{iF})(iTarget, iValid) = mean(extreme_dprimeDisIdx);
             extreme_disdErr.(Disfieldnames{iF})(iTarget, iValid) = extreme_disdStd.(Disfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
             extreme_discStd.(Disfieldnames{iF})(iTarget, iValid) = std(extreme_cDisIdx);
             ...
             disdStd_over2N.(Disfieldnames{iF})(iTarget, iValid) = std(over2N_dprimeDisIdx);
             disd_over2N.(Disfieldnames{iF})(iTarget, iValid) = mean(over2N_dprimeDisIdx);
             disdErr_over2N.(Disfieldnames{iF})(iTarget, iValid) = disdStd_over2N.(Disfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
             discStd_over2N.(Disfieldnames{iF})(iTarget, iValid) = std(over2N_cDisIdx);
             disc_over2N.(Disfieldnames{iF})(iTarget, iValid) = mean(over2N_cDisIdx);
             discErr_over2N.(Disfieldnames{iF})(iTarget, iValid) = discStd_over2N.(Disfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
      ...
      for iF = 2:numel(Detfieldnames)
          for iValid = 1:numel(Validities)
              for iSub = 1:length(subs)
                  ...
                  % det subsampled over2N
                  dprimeDetIdx_sub_over2N = [dprimeDetIdx_sub_over2N dataAll(iSub).detd_sub_over2N.(Detfieldnames{iF})(iTarget,iValid)]; % subsampled valid conditions dprime
                  detd_sub_over2N_scatterplot.(Detfieldnames{iF})(iValid, iSub, iTarget) = dataAll(iSub).detd_sub_over2N.(Detfieldnames{iF})(iTarget,iValid);
              ...
              detdStd_sub_over2N.(Detfieldnames{iF})(iTarget, iValid) = std(dprimeDetIdx_sub_over2N);
              detd_sub_over2N.(Detfieldnames{iF})(iTarget, iValid) = mean(dprimeDetIdx_sub_over2N);
              detdErr_sub_over2N.(Detfieldnames{iF})(iTarget, iValid) = detdStd_sub_over2N.(Detfieldnames{iF})(iTarget,iValid)/sqrt(length(subs));
              detcStd_sub_over2N.(Detfieldnames{iF})(iTarget, iValid) = std(critDetIdx_sub_over2N);
...
%% swapping over incorrect trials
...
for iContrast = 1:numel(contrastConds)-1
    ...
    for iTarget = 1:2
        for iValid = 1:3
            b = bar(xcoords(iContrast, iValid, iTarget),  Acc.NTErrmean(iContrast, iValid, iTarget));
            hold on 
            for iSub = 1:15
                s = scatter(xcoords_scatter(iValid, iSub, iTarget), swap_Err_Acc_scatterplot.(contrasts{iContrast})(iValid,iSub,iTarget));
                    s.MarkerEdgeColor = [1 1 1];
                    if iTarget == 1
                        s.MarkerFaceColor = fp.grey;
                    elseif iTarget == 2
            ...
            kt_figureStyle();
            errorbar(xcoords(iContrast, iValid, iTarget),Acc.NTErrmean(iContrast, iValid, iTarget),Acc.NTErrerr(iContrast, iValid, iTarget), '.k', 'MarkerSize', 0.01, 'CapSize', 0, 'LineWidth', 1.75)
            if iContrast == 1 || iContrast == 3
                if iTarget == 1
                    b.FaceColor = fp.grey;
                    b.EdgeColor = fp.grey;
    ...
    %     ]), 'FontSize', 4);
    % label_y = ylabel(sprintf('proportion of \nreporting non-target feature (%%) \n'));
    label_y = ylabel(sprintf('Error due to reports of \nnon-target feature ( %%) \n'));
    ylim([0 100]) % original [0 20]
    label_y.Position(1) = 0.45;
    ax = gca;
    set(gca, 'ytick', 0:10:100) % original 0:5:20
    hold on
    xlim([0.5 2.5])
    xticks([0.7778 1 1.222 1.7778 2 2.2222])
    set(gca, 'xticklabel', {'V', 'N', 'I'}, 'FontSize', 9.5)
