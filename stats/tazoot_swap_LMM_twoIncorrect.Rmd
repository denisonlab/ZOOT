...
data$targetContrast <- factor(data$targetContrast, levels = c("1", "0"), ordered = FALSE)
data$nontargetContrast <- factor(data$nontargetContrast, levels = c("1", "0"), ordered = FALSE)
data$Target <- factor(data$Target, levels = c("1", "2"), ordered = FALSE)
data$Validity <- factor(data$Validity,levels = c("1", "2", "3"), ordered = FALSE) # 
...
ggplot(data, aes(x = Validity, y = twoIncorrect, color = TNTC)) + 
...
print(aov.swap)
```
## combined contrast - T1
```{r swap combined contrast T1}
d <- data %>% mutate(combinedContrast = paste(targetContrast, nontargetContrast, sep = "_"))
d <- d %>% 
  mutate(combinedContrast = factor(combinedContrast,
                                   levels =  c("1_1", "1_0", "0_1"),
                                   ordered = TRUE)) %>% filter(Target==1)
swap.lmm.lme4 <- lmer(twoIncorrect ~ Validity * combinedContrast + (1|SID), data = d)
summary(swap.lmm.lme4)
aov.swap <- Anova(swap.lmm.lme4)
print(aov.swap)
```

## combined contrast - T1 - TPNA
```{r swap combined contrast T1}
d <- data %>% mutate(combinedContrast = paste(targetContrast, nontargetContrast, sep = "_"))
d <- d %>% 
  mutate(combinedContrast = factor(combinedContrast,
                                   ...
                                   ordered = TRUE)) %>% 
  filter(Target==1) %>% 
  filter(combinedContrast=="1_0")
swap.lmm.lme4 <- lmer(twoIncorrect ~ Validity + (1|SID), data = d)
summary(swap.lmm.lme4)
aov.swap <- Anova(swap.lmm.lme4)
print(aov.swap)
```

## combined contrast - T1 - TANP
```{r swap combined contrast T1}
d <- data %>% mutate(combinedContrast = paste(targetContrast, nontargetContrast, sep = "_"))
d <- d %>% 
  mutate(combinedContrast = factor(combinedContrast,
...
## combined contrast - T2 
```{r swap combined contrast T2}
d <- data %>% mutate(combinedContrast = paste(targetContrast, nontargetContrast, sep = "_"))
d <- d %>% 
  mutate(combinedContrast = factor(combinedContrast,
                                   levels =  c("1_1", "1_0", "0_1"),
                                   ordered = TRUE)) %>% filter(Target==2)
swap.lmm.lme4 <- lmer(twoIncorrect ~ Validity * combinedContrast + (1|SID), data = d)
summary(swap.lmm.lme4)
aov.swap <- Anova(swap.lmm.lme4)
print(aov.swap)
...
## T1 target absent non-target present
```{r swap combined contrast}
d <- data %>% filter(combinedContrast=="1_0") %>% filter(Target==2)
df_clean <- na.omit(d)
swap.lmm.lme4 <- lmer(twoIncorrect ~ Validity + (1|SID), data = df_clean)
summary(swap.lmm.lme4)
aov.swap <- Anova(swap.lmm.lme4)
print(aov.swap)

# One-sample t-test comparing mean(x) to 5
# %>% filter(Validity==1)
x <- data %>% filter(Target==2) %>% filter(combinedContrast=="1_1") %>%
  group_by(SID) %>% summarise(mean_swapRate = mean(twoIncorrect, na.rm = TRUE))

t.test(x$mean_swapRate, mu=50, alternative = "two.sided")
```
...
swapTPNP.lmm.lme4 <- lmer(twoIncorrect ~ Validity + (1|SID), data = d)
summary(swapTPNP.lmm.lme4)
aov.swapTPNP <- Anova(swapTPNP.lmm.lme4)
print(aov.swapTPNP)
```
