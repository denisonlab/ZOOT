...
% function [data] = zoot_main(s)
% Runs trial sequence for TA-zoot
% 
% JM & KT
% december 2023
...
% Get the color codes for white, black, and grey
...
Screen('TextFont', window, p.font); % setup displayed text
...
% Full factorial design - creates matrix of trial conditions for full session
...
% generate gratings - 4 gratings (differ in phase only, set tilt and contrast later)
for i=1:numel(p.phases) % we will just make four gratings of different phases, to be used across T1 and T2
    contrast = 1; 
    grating = rd_grating(pixelsPerDegree, p.gratingImSize, p.gratingSF, 0, p.phases(i), contrast); % creates grating 
    switch p.aperture % add aperture
        case 'cosine'
...
% Check subject data folder for any experiment files 
listing = dir(data.sesDir);
count = 1;
dataFileNames = []; 
...
if s.exptStage == 4 || s.exptStage == 5
    if isempty(dataFileNames)
        ...
        dataPrevious = load(dataFileNames{end}); % otherwise load the data w latest  time stamp and find the last trial that was left off
...
while iTrial <= size(trialOrder, 2)
    ...
    if ~stopThisTrial
        ...
        timeT1 = Screen('Flip', window, timePrecue_visual + p.precueSOA - slack);
        if p.eyeTracking
            Eyelink('Message', 'T1')
        end
